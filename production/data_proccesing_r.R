...
apod_data$sexo_apod <- apod_label(x = est_data$sexo_apod,label = "Sexo")
...
#SPFD
est_data$pp_voto_presi <- set_na(est_data$pp_voto_presi, na = c(9), drop.levels = TRUE, as.tag = FALSE)
...
#Sexo estudiantes a variable dummy y factor Hombre=1, Mujer = 0 
...
### OPERACIONALIZACIÓN DE VARIABLES y CREACIÓN DE ÍNDICES
#Orden en variables de aprendizaje civico (data$civ_)
...
#Se genera un alpha de cronbach para saber si los indicadores se comportan como índice
alfa <- alpha(df_civico)
alfa
...
data$pp_voto_presi <- car::recode(data$pp_voto_presi, "1=1;2=0")
...
#Análisis Factorial Exploratorio de Variables de organizaciones voluntarias (data$org) y de actividades
#políticas (data$act)
df_AFE <- data.frame(data$org_asocprof, data$org_caridad, data$org_cpadres, 
                     data$org_iglesia, data$org_juntavecinos, data$org_ppol,
                     data$org_sindicato, data$act_voluntariado, data$act_marcha, 
                     data$act_peticion, data$act_reu_sociopol)
